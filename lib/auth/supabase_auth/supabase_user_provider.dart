...
class RentLinkSupabaseUser extends BaseAuthUser {
  RentLinkSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> rentLinkSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RentLinkSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
